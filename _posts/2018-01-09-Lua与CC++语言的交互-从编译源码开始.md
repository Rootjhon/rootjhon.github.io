...
>  0. `VS` 新建 一个 `C++` **静态库** 项目。
>  0. 在 **解决方案管理器** 面板，将 下载的`Lua5.3.4` 源码的 **`src`目录** 下的 **`.h`**添加到 **头文件** 里。
>  0. 在 **解决方案管理器** 面板，将 下载的`Lua5.3.4` 源码的 **`src`目录** 下的 **`.C`**添加到 **源文件** 里。

## 修改项目配置

> 0. 在 **解决方案管理器** 面板，`右击 -> 属性 -> C/C++ -> 常规 -> 附加包含目录 ` 添加 下载的`Lua5.3.4` 源码的 **`src`目录**  。如下图所示：
>
> > ![](https://fastly.jsdelivr.net/gh/Rootjhon/img_note@empty/16763668113431676366810745.png)

> 0.  在属性页面 `C/C++ -> 高级 -> 编译为` 选择 : `编译为 C 代码(/TC)` 。如下图所示
>
> > ![](https://fastly.jsdelivr.net/gh/Rootjhon/img_note@empty/16763668193431676366819177.png)

> 0. 开始编译，可以看到控制台输出的库文件目录。
>
...
> 0. 在 **解决方案管理器** 面板，`右击 -> 属性 -> C/C++ -> 常规 -> 附加包含目录 ` 添加 下载的`Lua5.3.4` 源码的 **`src`目录** 。

> 0. 添加上一步编译好的库，`连接器 -> 常规 -> 附加库目录` ，选择 你对应的`lib`所在路径。
> 0.  在`连接器 -> 输入 -> 附加依赖项` ，填上你编译出来的`lib` 库文件名。
> ![](https://fastly.jsdelivr.net/gh/Rootjhon/img_note@empty/16763668323531676366832063.png)

##编写lua测试

> 0. 在源文件里新建一个`halo.lua` 文件。
> 0. 在 `main.cpp` 或 对应的 `.h` 文件 里，引用头文件。
...
> 0. `halo.lua` 里编写 `print("halo world")`

> 0.  在`main` 函数里编写后，运行编译。
